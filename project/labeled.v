...
Inductive fitrans: pi -> nat -> pi -> Prop := 
  | IN   (n : nat) (P: pi): 
    fitrans (In n P) n P
  | PAR2 (n : nat) (P Q R : pi):
    fitrans P n R -> fitrans (Par P Q) n (Par R (push 0 Q))
  | RES3 (n : nat) (P Q : pi) :
    fitrans P (n+1) Q -> fitrans (Res P) n (Res (swap 0 Q))
  ...
  with btrans : pi -> nat -> pi -> Prop :=
    ...
    | PAR2B  (n : nat) (P Q R : pi):
      btrans P  n R -> btrans (Par P Q) n (Par R (push 0 Q))
    | RES3B (n : nat) (P Q : pi) :
      btrans P (n+1) Q -> btrans (Res P) n (Res (swap 0 Q))
